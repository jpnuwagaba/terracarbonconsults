...
const dropdownContent: Record<DropdownKey, DropdownContent> = {
  ...
  'focus-areas': {
    heading: 'Focus Areas',
    description:
      'We work across key sectors where carbon markets can deliver measurable climate impact and sustainable development benefits, from nature-based land systems to clean energy, circular waste solutions, and low-emission mobility.',
    items: [],
    imageSrc:
...
const NavbarClient = ({ sectors }: NavbarClientProps) => {
  ...
  const dynamicDropdownContent = useMemo<Record<DropdownKey, DropdownContent>>(() => {
    const firstSectorWithImage = sectors.find((sector) => sector.image);
    ...
    return {
      ...
      'focus-areas': {
        ...
        items: sectors.map((sector, index) => ({
          ...
          icon: sectorDropdownIcons[index % sectorDropdownIcons.length],
        })),
        imageSrc: firstSectorWithImage?.image ?? dropdownContent['focus-areas'].imageSrc,
  ...
  const activeSections = useMemo(() => {
    ...
    return dynamicDropdownContent[activeDropdown];
  }, [activeDropdown, dynamicDropdownContent]);
            ...
            >
              <nav className="space-y-3">
                {navItems.map((item) => {
                  ...
                  const content = dynamicDropdownContent[item.key];
